...
src/
  ...
  words.js          こどもモードの文言（おとな／こどもの対応表）（6.5）
  ui/               各画面
...
- **こどもモードでは更新バナーを出しません。** 代わりに、セット実施中でなければ
  待機中の Service Worker をそのまま有効化します（`src/pwa.js` の `notifyUpdate`）。
  バナーを出さないだけだと端末がいつまでも古いままになるためです。
- **こどもモードの文言は `src/words.js` の表に集約しています。** `WORDS` が
  ［おとなモードの言い方, こどもモードの言い方］の対で、画面側は `w('stop')` のように
  キーで引きます。言い回しを直すときはこの表だけを触れば済み、おとなモードの文言は
  変わりません。もともとどちらでも読める語（「わかる」「おぼえた」など）は表に入れず、
  そのまま使っています。
